...
import { analyzeTranscriptForSegments } from './services/localSegmentService';
import { transcribeVideo } from './services/localWhisperService';
...
const App: React.FC = () => {
  ...
  const [isAnalyzingSegments, setIsAnalyzingSegments] = useState<boolean>(false);
  ...
  const handleStartLocalProcessingChain = useCallback(async () => {
    ...
    let transcript = '';
    try {
      transcript = await transcribeVideo(videoFile);
      setTranscription(transcript);
    } catch (e) {
      console.error('Transcription error:', e);
      setError(`Failed to transcribe video. ${e instanceof Error ? e.message : 'Unknown error'}`);
      setIsTranscribing(false);
      return;
    }
    setIsTranscribing(false);

    // Analyze the transcript using the local segmentation logic
    setIsAnalyzingSegments(true);
    try {
      const segments = await analyzeTranscriptForSegments(transcript);
      setSuggestedSegments(segments);
      setCurrentStep(AppStep.SelectSegment);
      if (segments.length === 0) {
        setError("The local analysis could not identify distinct segments from the transcript. Try again or adjust the transcript.");
      }
    } catch (e) {
      console.error("Error analyzing transcript:", e);
      setError(`Failed to analyze transcript. ${e instanceof Error ? e.message : 'Unknown error'}`);
    } finally {
  ...
  const getStepName = (step: AppStep): string => {
    switch(step) {
      ...
      case AppStep.GenerateTranscript: return "Generate Transcript & Analyze Segments";
    ...
    <div className="min-h-screen flex flex-col items-center p-4 md:p-8 bg-gradient-to-br from-gray-900 to-slate-800">
      <header className="w-full max-w-5xl mb-8 text-center">
        ...
        <p className="text-gray-400 mt-2 text-sm md:text-base">
          AI-assisted workflow for identifying key video moments using local processing concepts.
        </p>
      </header>
        ...
        <div className="bg-gray-800 shadow-2xl rounded-xl p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <div className="bg-gray-700 p-4 rounded-lg shadow-inner">
                ...
                <div className="mt-4 p-3 bg-yellow-900 bg-opacity-30 border border-yellow-700 rounded-md text-xs text-yellow-300">
                  ...
                  Transcription is performed locally with Whisper. Video processing (e.g., auto-framing, rendering) still relies on your own tools.
                </div>
              </div>
            </div>
          )}
          ...
          {currentStep === AppStep.GenerateTranscript && videoFile && (
            <TranscriptionPane
              ...
              isAnalyzing={isAnalyzingSegments}
              transcript={transcription}
            />
          )}
          
          {currentStep === AppStep.SelectSegment && transcription && !isAnalyzingSegments && !isTranscribing && (
            suggestedSegments.length > 0 ? (
              <div className="space-y-4">
                <h3 className="text-xl font-semibold text-pink-400 flex items-center"><LightBulbIcon className="w-6 h-6 mr-2"/>AI Suggested Segments</h3>
                <p className="text-gray-400 text-sm">Select a segment to configure as a clip. These are based on the local analysis of the transcript.</p>
              ...
              <div className="text-center py-4 text-gray-400">
                ...
                <p>No segments were suggested by the local analysis from the transcript.</p>
                <p>You can try running the "Generate Transcript & Analyze Segments" step again or check your transcription quality.</p>
                <button 
      ...
      <footer className="w-full max-w-5xl mt-12 text-center text-gray-500 text-xs">
        ...
        <p>This tool simulates a workflow involving local AI (like Whisper for transcription, FFmpeg for processing) and performs transcript analysis locally. Actual local processing is not performed by this web UI.</p>
      </footer>
    </div>
  );
};
